...
def _validate_request(request: Any) -> tuple[str, str, str, float]:
    ...
    if "language" in request and request["language"] != "ru":
        raise ValueError("language must be 'ru'")

    variant = request.get("variant", DEFAULT_VARIANT)
    if not isinstance(variant, str) or not variant:
        raise ValueError("variant must be a non-empty label")
    ...
    return model_path, audio_path, variant, chunk_seconds
...
def _normalize_transcript(result: Any) -> str:
    ...
    if isinstance(result, list):
        if not result:
            raise TypeError("model returned an empty transcript list")
        return _normalize_transcript(result[0])
...
def _transcribe_offline_fixed_chunks(
    model: Any, audio_path: str, chunk_seconds: float
) -> str:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        for chunk_index, start in enumerate(range(0, len(audio), chunk_frames)):
            ...
            sf.write(chunk_path, chunk, _SAMPLE_RATE)
...
def execute(
    ...
    *,
    ...
    """Execute one offline GigaAM transcription request."""
    try:
        model_path, audio_path, variant, chunk_seconds = _validate_request(request)
    ...
    try:
        ...
        try:
            raw_transcript = _transcribe_offline_fixed_chunks(
                model, audio_path, chunk_seconds
            )
